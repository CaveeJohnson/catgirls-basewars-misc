...
do
	...
	local volume = CreateClientConVar("sat_volume", "0.4", true, false, "Satellite visualizer volume.")
	cvars.AddChangeCallback("sat_volume", function()
		if not IsValid(g_vistest_stream) then return end
		g_vistest_stream:SetVolume(volume:GetFloat())
	end, "stream_updater")
	...
	function vistest_play(url)
		...
		sound.PlayURL(url, "noblock", function(stream)
			...
			if not IsValid(stream) then LocalPlayer():ChatPrint("invalid stream: " .. url) return end

			stream:SetVolume(volume:GetFloat())
		end)
	end

	local fuck_off_fov = CreateClientConVar("sat_no_fov_fuckery", "0", true, false, "Should the satellite not fuck with the FOV?")
	local fucking_fov = CreateClientConVar("sat_fov_fuck_factor", "1", true, false, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA?")

	function ext:CalcView(ply, origin, angles, fov)
		if fuck_off_fov:GetBool() then return end
		...
		local view = {}
			view.origin = origin
			view.angles = angles
			view.fov    = fov + (amp * fucking_fov:GetFloat())
		return view
	end
end
...
function ext:PostDrawSkyBox()
	self.lastDrawing3D = CurTime()
	self:render()
end

function ext:PostDraw2DSkyBox()
	if self.lastDrawing3D and self.lastDrawing3D + 0.1 >= CurTime() then
		return
	end
...
hook.Remove("PostDrawSkyBox", "bw18-sat")
hook.Remove("PostDraw2DSkyBox", "bw18-sat")
for name, f in pairs(ext) do
	if isstring(name) and name:match("^%u") then print("hooking ", name, f) hook.Add(name, "bw18-sat", function(...) return f(ext, ...) end) end
end
