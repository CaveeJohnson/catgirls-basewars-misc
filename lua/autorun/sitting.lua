...
if SERVER then
	...
	net.Receive(tag, function(_, ply)
		ply.sit_prev_weapon = IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() or "hands"
		ply:Give("hands")	-- should have this by default?
		ply:SelectWeapon("hands")
		ply:SetActiveWeapon(ply:GetWeapon("hands"))
...
hook.Add("PlayerSwitchWeapon", tag, function(ply, owep, nwep)
	if ply:IsSitting() then
		if SERVER and (ply.Unrestricted or ply:GetNWBool("Unrestricted")) then return end
		if nwep:GetClass() == "hands" then return true end
