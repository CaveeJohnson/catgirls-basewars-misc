...
local function message(...)
	...
	if relayChat:GetBool() then
		if tetra then
			tetra.rpc(nil, "chat.AddText", ...)
		elseif chat then
			chat.AddText(...)
		end
	end
...
hook.Add("PostPlayerInfoSpawn", "EPOE.Information.PostPlayerInfoSpawn", function(ply)
	...
	if not city or city == "ERROR" or city == "N/A" then
		...
		MsgC(geoColor, "[Location] ", normalColor, ply:Name() .. " is from ", countColor, country, normalColor, ".")
	else
		MsgC(geoColor, "[Location] ", normalColor, ply:Name() .. " is from ", cityColor, city, normalColor, " in ", countColor, country, normalColor, ".")
