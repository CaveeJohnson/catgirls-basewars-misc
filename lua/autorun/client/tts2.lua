...
function utf8.tohexcodes(str)
	local tbl = {}

	for uchar in string.gmatch(str, ".") do
		tbl[#tbl+1] = string.format("%%%X", string.byte(uchar))
	end

	return table.concat(tbl, "")
end

function tts.escape(text)
	return utf8.tohexcodes(text)
end
...
function tts.say(text, accent)
	...
	sound.PlayURL(tts.main_url ..  tts.escape(text) .. "&tl=" .. accent, "noblock mono", _genericHandle)
end
...
function tts.sayply(ply, text, accent)
	...
	local accent = tts.getAccent(accent)
	sound.PlayURL(tts.main_url ..  tts.escape(text) .. "&tl=" .. accent, "noblock mono 3d", _handleToPlayer(ply))
end
